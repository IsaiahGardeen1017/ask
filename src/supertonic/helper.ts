import { InferenceSession, Tensor } from 'npm:onnxruntime-node';
...
class UnicodeProcessor {
  ...
  _preprocessText(text: string): string {
    ...
    const exprReplacements: { [key: string]: string } = {
      '@': ' at ',
      'e.g.,': 'for example, ',
      'i.e.,': 'that is, ',
    };
...
export class TextToSpeech {
  ...
  async _infer(
    ...
    if (textList.length !== style.ttl.dims[0]) {
      throw new Error(
        'Number of texts must match number of style vectors (batch size)'
      );
    }
    const bsz = textList.length;
    const { textIds, textMask } = this.textProcessor.call(textList);
    const textIdsShape: number[] = [bsz, textIds[0].length];
    const textMaskShape: number[] = [bsz, 1, textMask[0][0].length];
    ...
    const textEmbTensor = textEncResult.text_emb as Tensor;

    let { noisyLatent, latentMask } = this.sampleNoisyLatent(durOnnx);
    const latentShape: number[] = [
      bsz,
      noisyLatent[0].length,
      noisyLatent[0][0].length,
    ];
    const latentMaskShape: number[] = [bsz, 1, latentMask[0][0].length];

    const latentMaskTensor = arrayToTensor(latentMask, latentMaskShape);

    const totalStepArray = new Array<number>(bsz).fill(totalStep);
...
async function loadOnnx(
  ...
  return await InferenceSession.create(onnxPath, opts);
}

/**
 * Load all ONNX models for TTS
 */
async function loadOnnxAll(
  onnxDir: string,
  opts: InferenceSession.SessionOptions
): Promise<{
  dpOrt: InferenceSession;
...
export function loadVoiceStyle(
  ...
  const ttlStyle = new Tensor('float32', ttlFlat, [bsz, ttlDim1, ttlDim2]);
  const dpStyle = new Tensor('float32', dpFlat, [bsz, dpDim1, dpDim2]);

  return new VoiceStyle(ttlStyle, dpStyle);
}
 ...
 */
function arrayToTensor(array: number[][][] | number[][] | number[], dims: number[]): Tensor {
  // Flatten the array
  const flat = (array as any).flat(Infinity) as number[];
  return new Tensor('float32', Float32Array.from(flat), dims);
}

/**
 * Convert 2D int array to ONNX tensor
 */
function intArrayToTensor(array: number[][], dims: number[]): Tensor {
