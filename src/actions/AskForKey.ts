import { ActionData as ActionData } from '../index.ts';
...
export async function askForKey(reqData: ActionData) {
	const config = await getConfiguration(reqData.configPath);
	log(
		termFmt('\nYou must enter a Gemini API key to use this tool - ', 'black') +
			termFmt('https://aistudio.google.com/api-keys', 'blue'),
	);
	const key = prompt('Gemini key:');
	if (key) {
		config.apiKey = key;
		await writeConfigFile(reqData.configPath, config);
		log('saved key', 'blue');
	} else {
